<template>
 <div class="tdee-calculator">
   ...
   <TDEEResultComponent v-if="result" :result="result" />
 </div>
</template>
...
import TDEEResultComponent from './TDEEResult.vue';
import type { TDEEFormData } from '../types';
import type { TDEEResult as TDEEResultType } from '../types';

const formRef = ref<FormInstance>();
const tdeeStore = useTDEEStore();

const isLoading = computed(() => tdeeStore.isLoading);
const lastInput = computed(() => tdeeStore.lastInput);
const activityLevels = computed(() => tdeeStore.activityLevels);

const result = ref<TDEEResultType | null>(null);
...
const handleCalculate = async () => {
 ...
 try {
   ...
   result.value = await tdeeStore.calculate(input);
...
onMounted(async () => {
 await Promise.all([
   tdeeStore.loadLastInput(),
   tdeeStore.loadActivityLevels()
 ]);
